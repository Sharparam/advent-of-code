...
class Map
  ...
  def set_routine(routine, continuous = false)
    ...
    input :newline
    routine[:b].chars.map(&:ord).each { |v| @cpu.input! v }
    input :newline
    routine[:c].chars.map(&:ord).each { |v| @cpu.input! v }
    input :newline
    input continuous ? :yes : :no
    input :newline
  end
  ...
  def draw
    puts @cpu.output[0...-1].map(&:chr).join
  end
end
...
# Find intersections
part1 = map.grid.keys.sum do |pos|
  others = [
    map.grid[pos + TRANSFORMS[:north]],
    map.grid[pos + TRANSFORMS[:south]],
    map.grid[pos + TRANSFORMS[:west]],
    map.grid[pos + TRANSFORMS[:east]]
  ]

  if map.grid[pos] == :scaffold && others.count(:scaffold) == 4
    pos.x * pos.y
  else
    0
  end
